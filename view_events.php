...
// Fetch events with schedule info
$result = mysqli_query($conn, "
    SELECT se.event_id, se.event_name, se.event_type, es.schedule_id, es.event_date, es.event_time, es.location
    FROM sports_events se
    LEFT JOIN event_schedule es ON se.event_id = es.event_id
...
<head>
  ...
  <title>Manage Events - Anti-Gravity Engine</title>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Primary Actions */
    .btn-primary {
      background: linear-gradient(135deg, rgba(0, 119, 255, 0.9), rgba(136, 0, 255, 0.9));
      color: white;
    ...
    <div class="card">
      ...
      <div class="header-area">
        <div class="title-group">
          <h3><i class="fa fa-layer-group" style="color:var(--neon-blue);"></i> Registry Overview</h3>
          <p>Total Protocols: <b><?php echo $total; ?></b></p>
        </div>
        
        <div class="controls-wrapper">
          <a href="add_event.php" class="btn btn-success">
            <i class="fa fa-plus-circle"></i> Initialize Event
          </a>
      ...
      <div class="table-container">
        <table class="table">
          ...
          <tbody id="eventTable">
            ...
            <tr>
              <td><span style="color:var(--neon-blue);">#<?php echo $row['event_id']; ?></span></td>
              <td><?php echo $row['event_name']; ?></td>
              <td><?php echo $row['event_type']; ?></td>
              <td><?php echo $row['event_date'] ?? '<span style="color:#aaa;">Unassigned</span>'; ?></td>
              <td><?php echo $row['event_time'] ?? '<span style="color:#aaa;">-</span>'; ?></td>
              <td><?php echo $row['location'] ?? '<span style="color:#aaa;">-</span>'; ?></td>
              
              <td class="actions-cell">
                  <!-- Edit Event -->
                  <a href="edit_event.php?id=<?= $row['event_id'] ?>" class="btn btn-warning btn-sm" title="Edit Event">
                    <i class="fa fa-edit"></i>
                  </a>
 
                  <?php if($row['schedule_id']) { ?>
                      <!-- Edit/Delete Schedule if exists -->
                      <a href="edit_schedule.php?id=<?= $row['schedule_id'] ?>" class="btn btn-primary btn-sm">
                        <i class="fa fa-clock"></i> Modify Schedule
                      </a>
                      <a href="delete_schedule.php?id=<?= $row['schedule_id'] ?>" class="btn btn-danger btn-sm" title="Remove Schedule">
                        <i class="fa fa-trash"></i>
                      </a>
                  <?php } else { ?>
                      <!-- Add Schedule button if not scheduled -->
                      <a href="add_schedule.php?event_id=<?= $row['event_id'] ?>" class="btn btn-success btn-sm">
                        <i class="fa fa-calendar-plus"></i> Bind Schedule
                      </a>
                  <?php } ?>
              </td>
            </tr>
