...
$upcoming     = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM event_schedule WHERE event_date >= CURDATE()"));
$completed    = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM event_schedule WHERE event_date < CURDATE()"));
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .sidebar::before {
      ...
      top: 0; left: 0; bottom: 0;
      width: 4px;
      background: linear-gradient(180deg, var(--red) 0%, var(--orange) 25%, var(--yellow) 50%, var(--green) 75%, var(--blue) 100%);
    }
    ...
    .sidebar-brand h2 span { color: var(--green); }
    ...
    .hero-lanes span { flex: 1; }
    .hero-lanes span:nth-child(1) { background: var(--blue); }
    .hero-lanes span:nth-child(2) { background: var(--green); }
    .hero-lanes span:nth-child(3) { background: var(--yellow); }
    .hero-lanes span:nth-child(4) { background: var(--orange); }
    .hero-lanes span:nth-child(5) { background: var(--red); }
    ...
    .stat-card-bar { height: 5px; width: 100%; }
 
    .stat-card--events .stat-card-value { color: var(--blue); }
    .stat-card--events .stat-icon { background: rgba(0,119,255,.1); color: var(--blue); }
    .stat-card--events .stat-card-bar { background: linear-gradient(90deg, var(--blue), #00d4ff); }
 
    .stat-card--participants .stat-card-value { color: var(--green); }
    .stat-card--participants .stat-icon { background: rgba(0,184,107,.1); color: var(--green); }
    .stat-card--participants .stat-card-bar { background: linear-gradient(90deg, var(--green), var(--yellow)); }
 
    .stat-card--upcoming .stat-card-value { color: var(--orange); }
    .stat-card--upcoming .stat-icon { background: rgba(255,107,0,.1); color: var(--orange); }
    .stat-card--upcoming .stat-card-bar { background: linear-gradient(90deg, var(--orange), var(--red)); }
 
    .stat-card--completed .stat-card-value { color: #7c3aed; }
    .stat-card--completed .stat-icon { background: rgba(124,58,237,.1); color: #7c3aed; }
    .stat-card--completed .stat-card-bar { background: linear-gradient(90deg, #7c3aed, #4f46e5); }
    ...
    .action-btn:nth-child(1) .action-icon { background: rgba(0,119,255,.1); color: var(--blue); }
    .action-btn:nth-child(2) .action-icon { background: rgba(255,190,0,.1); color: #c98b00; }
    .action-btn:nth-child(3) .action-icon { background: rgba(0,184,107,.1);  color: var(--green); }
    .action-btn:nth-child(4) .action-icon { background: rgba(232,52,28,.1);  color: var(--red); }
...
<aside class="sidebar">
  <div class="sidebar-brand" style="position: relative;">
    ...
    <div class="logo-eyebrow">User Portal</div>
    <h2>SPORTS<br/><span>MEET</span></h2>
  </div>
 
  <div class="sidebar-section">Main</div>
  <nav>
    <a href="user_dashboard.php" class="active"><i class="fa fa-home"></i> Dashboard</a>
    <a href="event_calendar.php"><i class="fa fa-calendar"></i> Event Calendar</a>
    <a href="live_scoreboard.php"><i class="fa fa-trophy"></i> Live Scoreboard</a>
    <a href="medal_table.php"><i class="fa fa-medal"></i> Medal Table</a>
 
    <div class="sidebar-nav-header">My Profile</div>
    <a href="event_list.php"><i class="fa fa-running"></i> Registration</a>
    <a href="my_events.php"><i class="fa fa-list"></i> My Events</a>
    <a href="user_results.php"><i class="fa fa-flag-checkered"></i> My Result</a>
    
    <div class="sidebar-nav-header">Session</div>
...
<div class="main">
  ...
  <div class="hero">
    <div class="hero-bg-number">02</div>
    ...
    <h1>HELLO<br/><em><?php echo strtoupper($_SESSION['user']); ?></em></h1>
    <p class="hero-sub">Ready to dominate the track? Here's how your sports meet is shaping up today.</p>
  ...
  <div class="content">
      ...
      <div class="stat-card stat-card--upcoming">
        <div class="stat-card-top">
          <div>
            ...
            <div class="stat-card-desc">Events scheduled next</div>
          </div>
      ...
      <div class="stat-card stat-card--completed">
        <div class="stat-card-top">
          <div>
            ...
            <div class="stat-card-desc">Finished matches</div>
          </div>
    ...
    <div class="actions-grid">
      <a href="event_calendar.php" class="action-btn">
      ...
      <a href="live_scoreboard.php" class="action-btn">
        ...
        <span class="action-label">Live Scoreboard</span>
      </a>
      <a href="medal_table.php" class="action-btn">
        <div class="action-icon"><i class="fa fa-medal"></i></div>
        <span class="action-label">Medal Table</span>
      </a>
    </div>
